...
class Scraper
{
    ...
    public function parse_server($server, $page=1, $to_page=1, $log)
    {
        if($page > $to_page) {
            return true;
        }
    ...
    private function parse_book($server, $a_href)
    {
        ...
        if(!empty($book) && $book->status == Book::INACTIVE) {
            return 1;
        }
        ...
        $book_id = $html_base->find($server->book_id_key)[0]->value;

        $book = new Book();
        $book->status = Book::ACTIVE;
        $book->server_id = $server->id;
        $book->server_book_id = $book_id;
        $book->url = $a_href;
        $book->image_source = $image_src;
        $book->image = $image;
        $book->title = $title;
        $book->slug = $slug;
        $description = trim($html_base->find($server->description_key)[0]->plaintext);
        ...
        foreach ($tags as $tag) {
            $book->addScraperTag(strtolower(trim(html_entity_decode($tag->plaintext))));
        }
    ...
    private function get_release_date($release_date) {
        ...
        $release_day_array = explode('-', $release_date_array[0]);
        if(strlen($release_day_array[0]) == 2
            && strlen($release_day_array[1]) == 2
            && strlen($release_day_array[2]) == 4) {
            $release_date = $release_day_array[2].'-'.$release_day_array[1].'-'.$release_day_array[0].' '.$release_date_array[1];
        }
        return $release_date;
    }

    private function get_chapters($book, $page=1, $skip=true, $dem=0) {
        $url = $book->server->list_chapters_url;
        $url = str_replace('{book_id}', $book->server_book_id, $url);
        $url = str_replace('{page}', $page, $url);
        ...
        foreach ($chapters as $num => $chapter) {
            ...
            $chapter_url_parsers = explode('/',$chapter_url);
            $server_chapter_id = 0;
            foreach($chapter_url_parsers as $parse) {
                $parse = str_replace('c','',$parse);
                if(is_numeric($parse)) {
                    $server_chapter_id = (int) $parse;
                    break;
                }
            }
            if($server_chapter_id == 0) {
                continue;
            }
            $chapter_url = $this->get_full_href($book->server, $chapter_url);
            ...
            $release_date_key = $book->server->release_date_key;
            while(substr($release_date_key,0,3) == '../') {
                $chapter = $chapter->parent();
                $release_date_key = substr($release_date_key,3);
            }
            $release_date = trim($chapter->find($release_date_key)[0]->plaintext);
            $release_date = $this->get_release_date($release_date);

            if(empty($book->release_date) || strtotime($book->release_date) < strtotime($release_date)) {
            ...
            if(empty($chapter)) {
                ...
                $chapter->stt = $dem;
                $chapter->server_chapter_id = $server_chapter_id;
